  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--black)' }}>
      ...
      <section style={{ padding: '100px 40px 60px', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <Link
            ...
            onMouseEnter={e => { (e.currentTarget as HTMLAnchorElement).style.background = 'rgba(0,230,118,0.07)' }}
      ...
      <div style={{ display: 'flex', gap: 0, borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)', flexWrap: 'wrap' }}>
        {[
          ...
          <div key={s.label} style={{ padding: '32px 40px', borderRight: i < 3 ? '1px solid var(--border)' : 'none', flex: '1 1 180px' }}>
            <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 44, color: 'var(--green)', letterSpacing: 1, lineHeight: 1 }}>
              {s.main}<span style={{ color: 'var(--green)' }}>{s.accent}</span>
            </div>
      ...
      <section style={{ padding: '80px 40px', maxWidth: 900 }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: '#555', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 12 }}>The Problem</div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 2 }}>
          {[
            ...
            <div key={s.title} style={{ background: 'var(--card)', border: '1px solid var(--border)', padding: '28px 24px' }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--muted)', lineHeight: 1.7, fontFamily: "'DM Sans', sans-serif" }}>{s.body}</div>
            </div>
          ))}
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 2 }}>
          {[
            ...
            <div key={tree.name} style={{ background: '#0e0d0c', border: '1px solid rgba(0,230,118,0.15)', padding: '28px 24px', fontFamily: "'DM Mono', monospace" }}>
              <div style={{ fontSize: 22, color: 'var(--green)', fontFamily: "'Bebas Neue', sans-serif", letterSpacing: 3, marginBottom: 4 }}>{tree.name}</div>
              <div style={{ fontSize: 9, color: '#555', letterSpacing: 2, marginBottom: 16, textTransform: 'uppercase' }}>{tree.sub}</div>
      ...
      {/* SIGNAL STACK */}
      <section style={{ padding: '0 40px 80px', maxWidth: 900 }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: '#555', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 12 }}>How It Scans</div>
        <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 52, letterSpacing: 2, color: 'var(--white)', marginBottom: 40 }}>
          4 SIGNAL TIERS.<br />
      ...
      {/* INFECTION STAGES */}
      <section style={{ padding: '0 40px 80px', maxWidth: 900 }}>
        <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: '#555', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 12 }}>Infection Staging</div>
        <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 52, letterSpacing: 2, color: 'var(--white)', marginBottom: 40 }}>
          STAGE I TO IV.<br />
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 2 }}>
          {STAGES.map(stage => (
            <div key={stage.roman} style={{ background: 'var(--card)', border: '1px solid var(--border)', padding: '24px 20px' }}>
              <div style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 48, color: 'rgba(0,230,118,0.2)', lineHeight: 1, marginBottom: 8 }}>
                {stage.roman}
              </div>
              ...
              <div style={{ fontSize: 12, color: 'var(--muted)', lineHeight: 1.6, fontFamily: "'DM Sans', sans-serif" }}>{stage.body}</div>
            </div>
          ))}
        </div>
      </section>
      ...
      {/* FIELD OBSERVATION */}
      <section style={{ padding: '0 40px 80px', maxWidth: 900 }}>
        <div style={{ background: '#0e0d0c', border: '1px solid rgba(0,230,118,0.2)', padding: '48px 40px', position: 'relative', overflow: 'hidden' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: '#555', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 16 }}>Field Intelligence Loop</div>
      ...
      <section style={{ padding: '0 40px 80px', maxWidth: 900 }}>
        <div style={{ background: 'var(--card)', border: '1px solid var(--border)', padding: '48px 40px' }}>
          <div style={{ fontFamily: "'DM Mono', monospace", fontSize: 11, color: '#555', letterSpacing: 3, textTransform: 'uppercase', marginBottom: 16 }}>Why Anathema</div>
          <h2 style={{ fontFamily: "'Bebas Neue', sans-serif", fontSize: 44, letterSpacing: 2, color: 'var(--white)', marginBottom: 20, maxWidth: 700 }}>
            THE PATHOGEN<br />
            <span style={{ color: 'var(--green)' }}>LEAVES TRACES.</span>
          </h2>
          <p style={{ fontSize: 14, color: 'var(--muted)', maxWidth: 600, lineHeight: 1.8, fontFamily: "'DM Mono', monospace", letterSpacing: 0.3 }}>
            In the AVP / Prometheus universe, an Anathema is an organism changed at a cellular level by Engineer pathogen — Chemical A0-3959X.91–15. It doesn't look like what it was before. But the pathogen leaves biological markers that reveal which strain infected it and how deep the mutation runs. Every insurance agent in this market has been changed — by the tree that recruited them, the carriers they were handed, the language they were trained to use. They don't announce it. But the markers are there. ANATHEMA reads them.
          </p>
        </div>
      </section>
