...
async function classifyAds(ads: any[], keyword: string): Promise<AdResult[]> {
  ...
  const batch = ads.slice(0, 30).map((ad, i) => ({
    index: i,
    advertiser: ad.pageName || ad.advertiserName || ad.page_name || 'Unknown',
    page_url: ad.pageUrl || ad.page_url || ad.advertiserUrl || null,
    text: (ad.adText || ad.ad_creative_bodies?.[0] || ad.body || ad.caption || '').slice(0, 400),
    spend: ad.spendRange || ad.spend?.upper_bound || null,
    start_date: ad.startDate || ad.ad_delivery_start_time || null,
    platforms: ad.platforms || ad.publisher_platforms || [],
  }))
...
[
  ...
  try {
    ...
    return batch.map(ad => ({
      ...
      recruitable: true,
      claude_take: 'Classification unavailable.',
    }))
  }
}
